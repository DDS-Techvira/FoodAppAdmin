...
<div class="row mb-none-30">
    <div class="col-xl-3 col-lg-5 col-md-5 mb-30">
        <div class="widget-two box--shadow2 b-radius--5 bg--white">
            <div class="card-body p-0">
                <div class="p-3 bg--white">
                    <div>
                        <img src="{{ getImage(imagePath()['doctor']['path'].'/'. $doctor->image,imagePath()['doctor']['size'])}}" alt="@lang('profile-image')" class="b-radius--10 w-100">
                    </div>
                    <div class="mt-15">
                        <h4>{{$doctor->name}}</h4>
        ...
        <div class="widget-two box--shadow2 b-radius--5 bg--white mt-30">
            <div class="card-body">
                <h5 class="mb-20 text-muted">@lang('Doctor information')</h5>
                <ul class="list-group">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Working Sector')
                        <span class="font-weight-bold">{{$doctor->sector->name}}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Location')
                        <span class="font-weight-bold">{{$doctor->location->name}}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Doctor Fees')
                        <span class="font-weight-bold">{{$doctor->fees}} {{$general->cur_text}}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @lang('Contact No')
    ...
    <div class="col-xl-9 col-lg-7 col-md-7 mb-30">
        <form action="{{ route('patient.appointment.store',$doctor->id) }}" method="post">
            @csrf
                ...
                <div class="card-body p-0">
                    <div class="p-3 bg--white">
                        ...
                        <div class="time-serial-parent mt-3">
                            ...
                            <a href="javascript:void(0)" class="btn btn--primary mr-2 mb-2 available-time item{{str_slug($item)}}" data-value="{{ $item }}" >{{ $item }}</a>
                            @endforeach
                        </div>
...
<script>
    ...
    (function ($) {
        ...
        $(document).on('change','select[name=booking_date]',function(){
            ...
            var url = "{{route('patient.appointment.booked.date')}}";
            var date = $(this).val();
