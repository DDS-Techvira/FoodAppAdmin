            ...
            <div class="card">
                <ul class="nav nav-pills py-3" id="pills-tab">
                    <li class="nav-item col">
                        <a class="nav-link text-center {{$tab == 'new' ? 'active' : ''}}" id="new-chats-tab" href="{{route('admin.chats.technical_support', 'new')}}">New</a>
                    </li>
                    <li class="nav-item col">
                        <a class="nav-link text-center {{$tab == 'ongoing' ? 'active' : ''}}" id="ongoing-chats-tab" href="{{route('admin.chats.technical_support', 'ongoing')}}">Ongoing</a>
                    </li>
                    <li class="nav-item col">
                        <a class="nav-link text-center {{$tab == 'closed' ? 'active' : ''}}" id="closed-chats-tab" href="{{route('admin.chats.technical_support', 'closed')}}">Closed</a>
                    </li>
                </ul>
                    ...
                    <div class="tab-pane fade show active" id="new-chats">
                        <div class="col">
                                    ...
                                    <a href="#" class="card mb-3 p-2 chat-card" data-toggle="modal" data-target="#modal-{{$chat->chat_code}}">
                                        <div class="media">
                                            ...
                                            <div class="media-body">
                                                <h5 class="mt-0">{{$chat->first_name}} {{$chat->last_name}}</h5>
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <a href="{{route('admin.chats.conversation.accept', ['technical-support', $chat->chat_code])}}" class="btn btn-primary">Accept</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            @elseif($tab == 'ongoing' Or $tab == 'closed')
                                @foreach($chats as $chat)
                                    <a href="{{route('admin.chats.conversation', ['technical-support', $tab, $chat->chat_code])}}" class="card mb-3 p-2 chat-card {{($tab == 'ongoing' && $chat->principal_replied === 1)?'chat-highlighted':''}}">
                                        <div class="media">
                                            @if($chat->principal)
                                                <img src="{{ asset('/assets/principal/images/profile/'.$chat->profile_image)}}" class="mr-3 img-thumbnail" width="64" />
                                            @else
                                                <img src="{{ asset('/assets/images/plaseholder.png')}}" class="mr-3 img-thumbnail" width="64" />
                                            @endif
                                            <div class="media-body">
