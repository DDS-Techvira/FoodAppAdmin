...
function uploadImage($file, $location, $size = null, $old = null, $thumb = null)
{
    ...
    try{
        Log::info('Try Start');
        $storagePath=Storage::disk('do_spaces')->putFile($location, $file, 'public');
         Log::info(basename($storagePath));
...
function uploadImageData($base64File, $location)
{
    $imageName = storage_path("app/public/temp/".uniqid() . time() . ".png");
    $fileData = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $base64File));
    file_put_contents($imageName, $fileData);

    $tmpFile = new File($imageName);
...
function imagePath()
{
    ...
    $data['image'] = [
        ...
        'default-user' => 'assets/images/plaseholder.png',
    ];
    $data['withdraw'] = [
        'method' => [
...
function diffDatesForHumans($start, $end)
{
    $lang = session()->get('lang');
    \Carbon\Carbon::setlocale($lang);
